    ...
    def _export_excel(self, calculation_history: List[Dict], include_charts: bool) -> BytesIO:
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            ...
            for method in methods:
                if method:
                    method_data = [calc for calc in calculation_history if calc.get('method') == method]
                    method_df = self._create_method_dataframe(method_data, method)
                    if not method_df.empty:
                        sheet_name = method.replace(' ', '_')[:31]  # Excel sheet name limit
                        method_df.to_excel(writer, sheet_name=sheet_name, index=False)
            
            # Chart data sheet (if requested)
            if include_charts:
                chart_data = self._extract_chart_data(calculation_history)
                if chart_data:
